...
public class SingleTreeNodeBias {
    public MCTSParamsBias params;

    private SingleTreeNodeBias parent;
    private SingleTreeNodeBias[] children;
    ...
    SingleTreeNodeBias(MCTSParamsBias p, Random rnd, int num_actions, Types.ACTIONS[] actions) {
        this(p, null, -1, rnd, num_actions, actions, 0, null);
    }

    private SingleTreeNodeBias(MCTSParamsBias p, SingleTreeNodeBias parent, int childIdx, Random rnd, int num_actions,
        ...
        children = new SingleTreeNodeBias[num_actions];
        totValue = 0.0;
    ...
    void mctsSearch(ElapsedCpuTimer elapsedTimer) {
        ...
        while (!stop) {
            ...
            SingleTreeNodeBias selected = treePolicy(state);
    ...
    private SingleTreeNodeBias treePolicy(GameState state) {

        SingleTreeNodeBias cur = this;
    ...
    private SingleTreeNodeBias expand(GameState state) {
        ...
        SingleTreeNodeBias tn = new SingleTreeNodeBias(params, this, bestAction, this.m_rnd, num_actions,
    ...
    private SingleTreeNodeBias uct(GameState state) {
        SingleTreeNodeBias selected = null;
        double bestValue = -Double.MAX_VALUE;
        for (SingleTreeNodeBias child : this.children) {
            ...
            // --------------------
            // Progressive Bias - customBias (0) and advancedBias (1)
            // --------------------
            double customBias = params.CUSTOM_HEURISTIC / (1 + (child.nVisits + params.epsilon));
            double advancedBias = params.ADVANCED_HEURISTIC / (1 + (child.nVisits + params.epsilon));
            double uctValue = childValue +
                    (params.K * Math.sqrt(Math.log(this.nVisits + 1) / (child.nVisits + params.epsilon))) + advancedBias;
    ...
    private double rollOut(GameState state) {
        ...
        // Decaying reward
        // --------------------
        double reward = rootStateHeuristic.evaluateState(state); // original reward
        double discount = 0.8; // short-term strategy (0) and long-term strategy (1)
        double rootDepth = thisDepth; // depth of current node
        double decayingFactor = Math.pow(discount, rootDepth); // discount factor

        // Debugging
//        NumberFormat formatter = new DecimalFormat("#0.0000");
//        System.out.println("Decaying Reward:\t" + formatter.format(reward * decayingFactor) + ",\tReward:\t" + formatter.format(reward));

        return (reward * decayingFactor);
    }
    ...
    private void backUp(SingleTreeNodeBias node, double result) {
        SingleTreeNodeBias n = node;
        while (n != null) {
            n.nVisits++;
            n.totValue += result;
    ...
    private boolean notFullyExpanded() {
        for (SingleTreeNodeBias tn : children) {
            if (tn == null) {
                return true;
            }
        }
